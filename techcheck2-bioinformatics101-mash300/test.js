...
var tests = {
    ...
    testExtractCodonsFromDNAFunction: function(codons){
        ...
        if(Array.isArray(codons) 
            && codons.every(matchesCorrectResult) 
            && codons.length === correctResult.length)
        {
            ...
            console.log(`[${correctResult.toString()}]`)
            console.log(`%cbut the function returned...`, `color: ${this.failColor}; font-size: ${this.fontSize};`);
            console.log(Array.isArray(codons) ? `[${codons.toString()}]` : codons)
        }
    ...
    testTranslateCodonsToAminosFunction: function(aminos){
        ...
        if(aminos !== undefined && aminos.every(matchesCorrectResult) && aminos.length === correctResult.length){
            ...
            console.log(`[${correctResult.toString()}]`)
            console.log(`%cbut the function returned...`, `color: ${this.failColor}; font-size: ${this.fontSize};`);
            console.log(Array.isArray(aminos) ? `[${aminos.toString()}]` : aminos)
        }
        console.log(`\n\n\n\n\n\n`)
    },
